        ...
        if ($this->getRequest()->isPost()) {
            ...
            if (!empty($login['username']) && !empty($login['password'])) {
                try {
                    ...
                    //custom code for check lock user
                    $sql = "SELECT * FROM `customer_entity` WHERE email= '$checkuser' AND `lock_expires` IS NOT NULL LIMIT 1";
                    $result = $connection->fetchAll($sql);
                    if(count($result) > 0){
                        $message = __(
                            'Your account has been locked temporarily due to multiple incorrect sign-in attempts. '
