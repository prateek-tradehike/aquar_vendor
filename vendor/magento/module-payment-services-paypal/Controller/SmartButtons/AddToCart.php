...
class AddToCart implements HttpPostActionInterface
{
     ...
     * @param CartInterfaceFactory $quoteFactor
     * @param Cart $cart
    ...
    public function execute() : ResponseInterface
    {
        ...
        try {
            ...
            if (!$product || !$product->getIsSalable()) {
                return $this->prepareErrorResponse(__('Product is out of stock. Please try again later.'));
            }
            ...
            if (!$this->customerSession->getCustomerId() &&
                ...
                return $this->prepareErrorResponse(__('To check out, please sign in with your email address.'));
            }
            ...
            $this->prepareErrorResponse($displayErrors);
        } catch (LocalizedException $e) {
            $this->prepareErrorResponse($this->escaper->escapeHtml($e->getMessage()));
        } catch (\Exception $e) {
            $this->logger->critical($e);
            $this->prepareErrorResponse(
                __('We can\'t add this item to your shopping cart right now. Please try again later.')
            );
        }
        $this->checkoutSession->setQuoteId($origQuoteId);
